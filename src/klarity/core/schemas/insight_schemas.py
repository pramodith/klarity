...
class Scores(BaseModel):
    overall_uncertainty: float = Field(
        ..., ge=0, le=1, description="Overall uncertainty score indicating the model's general uncertainty level"
    )
    confidence_score: float = Field(..., ge=0, le=1, description="Model's confidence in its analysis and predictions")
    hallucination_risk: float = Field(
        ..., ge=0, le=1, description="Risk level of model hallucination or fabrication in the response"
    )
